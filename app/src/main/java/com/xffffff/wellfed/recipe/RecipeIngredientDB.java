...
/**
 * The RecipeIngredientDB class is used to store and retrieve recipe
 * ingredient data from
...
public class RecipeIngredientDB {
    ...
    /**
     * Holds an instance of the collection of RecipeIngredients stored in the
     * FireStore db database
     */
    private final CollectionReference recipeIngredientsCollection;
    /**
     * Holds an instance of the collection of Ingredients stored in the
     * FireStore db database
     */
    ...
    /**
     * Adds a RecipeIngredient to the RecipeIngredient Collection. Adds an
     * Ingredient to Ingredient
     * Collection. The new RecipeIngredient document has a reference to the
     * new Ingredient document.
     * The RecipeIngredient has it's id set to the new RecipeIngredient
     * document id.
     *
     * @param ingredient The RecipeIngredient to be added
     * @return The id of the RecipeIngredient that was added
     * @throws InterruptedException If any transaction in the method was not
     * complete
     */
    public String addRecipeIngredient(Ingredient ingredient)
            throws InterruptedException {
        String newIngredientId = ingredientsCollection.document().getId();
        String newRecipeIngredientId =
                recipeIngredientsCollection.document().getId();
        ...
        db.runTransaction((Transaction.Function<Void>) transaction -> {
            ...
            DocumentReference newIngredientDocument =
                    ingredientsCollection.document(newIngredientId);
            DocumentReference newRecipeIngredientDocument =
                    recipeIngredientsCollection.document(newRecipeIngredientId);
    ...
    /**
     * Delete the RecipeIngredient and the referenced Ingredient document in
    ...
    public void delIngredient(String id) throws InterruptedException {
        ...
        final DocumentReference[] recipeIngredientDocument =
                new DocumentReference[1];
        ...
        db.runTransaction((Transaction.Function<Void>) transaction -> {
            ...
            recipeIngredientDocument[0] =
                    recipeIngredientsCollection.document(id);

            DocumentSnapshot recipeIngredientSnapshot =
                    transaction.get(recipeIngredientDocument[0]);

            ingredientDocument[0] =
                    recipeIngredientSnapshot.getDocumentReference("ingredient");
    ...
    /**
     * Update the RecipeIngredient document in the database with the new
     ...
     * complete
     */
    public void updateRecipeIngredient(Ingredient recipeIngredient)
            throws InterruptedException {
        ...
        DocumentReference recipeDocument =
                this.recipeIngredientsCollection.document(recipeIngredientId);
        ...
        db.runTransaction((Transaction.Function<Void>) transaction -> {
            ...
            DocumentSnapshot recipeIngredientSnapshot =
                    transaction.get(recipeDocument);
            ingredientDocument[0] =
                    recipeIngredientSnapshot.getDocumentReference("ingredient");
            return null;
        }).addOnSuccessListener(unused -> {
    ...
    /**
     * Get the RecipeIngredient Object from the recipe ingredient document
     * with the same id provided
     *
     * @param id A String of the id that corresponds to the id of the Recipe
     *           Ingredient document
     * @return RecipeIngredient corresponding to the document in the
     * collection if it does not exist
     * return null
     * @throws InterruptedException If any transaction in the method was not
     * complete
     */
    public Ingredient getRecipeIngredient(String id)
            throws InterruptedException {
        ...
        DocumentReference recipeIngredientDocument =
                recipeIngredientsCollection.document(id);
        final DocumentSnapshot[] recipeIngredientSnapshot =
                new DocumentSnapshot[1];
        final DocumentSnapshot[] ingredientSnapshot = new DocumentSnapshot[1];
        db.runTransaction((Transaction.Function<Void>) transaction -> {

            recipeIngredientSnapshot[0] =
                    transaction.get(recipeIngredientDocument);
        ...
        DocumentReference ingredientDocument =
                (DocumentReference) recipeIngredientSnapshot[0].get(
                        "ingredient");

        db.runTransaction((Transaction.Function<Void>) transaction -> {

            assert ingredientDocument != null;
        ...
        recipeIngredient.setAmount(Objects.requireNonNull(
                recipeIngredientSnapshot[0].getDouble("amount")));
        recipeIngredient.setDescription(Objects.requireNonNull(
                ingredientSnapshot[0].getString("description")));
        recipeIngredient.setUnit(Objects.requireNonNull(
                recipeIngredientSnapshot[0].getString("unit")));
        recipeIngredient.setCategory(Objects.requireNonNull(
                ingredientSnapshot[0].getString("category")));

        return recipeIngredient;
    }

    /**
     * Get the DocumentReference from RecipeIngredients collection for the
     * given id
     *
     * @param id The String of the document in RecipeIngredients collection
     *           we want
    ...
    /**
     * Gets all the corresponding RecipeIngredients from the Recipe
    ...
    public ArrayList<Ingredient> getRecipeIngredients()
        ...
        db.runTransaction((Transaction.Function<Void>) transaction -> {
            ...
            List<DocumentSnapshot> recipeIngredientSnapshots =
                    recipeIngredientsCollection.get().getResult()
                            .getDocuments();

            for (DocumentSnapshot recipeIngredientSnapshot : recipeIngredientSnapshots) {
                Ingredient recipe = null;
                try {
                    recipe = this.getRecipeIngredient(
                            recipeIngredientSnapshot.getId());
