...
public class ShoppingCartDBTest {
    ...
    private ArrayList<StorageIngredient> mockStorageIngredients() {
        ...
        storageIngredients.add(
                new StorageIngredient("Chicken", 1.0, "lb", "Pantry",
                        new Date(), "Chicken"));
        storageIngredients.add(
                new StorageIngredient("Beef", 1.0, "lb", "Pantry", new Date(),
                        "Beef"));
        storageIngredients.add(
                new StorageIngredient("Pork", 1.0, "lb", "Pantry", new Date(),
                        "Pork"));
        storageIngredients.add(
                new StorageIngredient("Fish", 1.0, "lb", "Pantry", new Date(),
                        "Fish"));
        storageIngredients.add(
                new StorageIngredient("Eggs", 1.0, "lb", "Pantry", new Date(),
                        "Eggs"));
     ...
     */
    @Test public void getIngredientsTest2() throws InterruptedException {
        // Create a CountDownLatch to wait for the async calls to finish
        CountDownLatch latch = new CountDownLatch(1);
        // Get one of the ingredients
        Ingredient ingredient = mockIngredients().get(0);
            ...
            shoppingCartDB.getShoppingCart((shoppingCart, success2) -> {
                ...
                shoppingCartDB.addIngredient(ingredient2,
                        (addedIngredient2, success3) -> {
                            ...
                            // Get the ingredients from the shopping cart
                            shoppingCartDB.getShoppingCart(
                                    (shoppingCart2, success4) -> {
                                        ...
                                        for (Ingredient ingredient3 : shoppingCart2) {
                                            if (ingredient3.getDescription()
                                                    .equals(addedIngredient.getDescription())) {
                                                foundIngredient1 = true;
                                            }
                                            if (ingredient3.getDescription()
                                                    .equals(addedIngredient2.getDescription())) {
